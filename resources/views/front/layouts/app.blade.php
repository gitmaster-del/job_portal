...
<div class="container "  >
    <p class="text-center text-white pt-3 fw-bold fs-6">© 2023 Ashish company, all right reserved</p>
</div>
</footer> 
...
$(document).ready(function() {
    $('#ProfilePicForm').submit(function(e) {
        ...
        if (fileInput.size > 500 * 1024) {
            $('#image-error').text('File size must be less than 500KB.');
            return;
        }
        ...
        $.ajax({
            ...
            success: function(response) {
                if (response.status) {
                    ...
                    $('.sidebar-profile-pic').attr('src', response.profile_pic_url);

                    var msgDiv = $('<div class="alert alert-success mt-2"></div>').text(response.message);
                    $('body').prepend(msgDiv);

                    setTimeout(function() {
                        msgDiv.fadeOut('slow', function() { $(this).remove(); });
                    }, 5000);

                    $('#ProfilePicForm')[0].reset();
                } else {
                    if (response.errors && response.errors.profile_pic) {
                        $('#image-error').text(response.errors.profile_pic[0]);
                    }
                }
            },
            error: function(xhr) {
